...
function saveTimeCard() {
  ...
  // 入力リセット
  if (isPaidLeave) {
    // 有給の場合は「名前」と「有給用 西暦」は残す
    // クリアする項目：出勤時間、退勤時間、複数日付（dateMulti）
  ...
  // UIのリセット
  if (!isPaidLeave) {
    // 通常勤務の場合のみ、チェックを外して、通常勤務用入力欄を表示
  ...
  // 有給の場合は、そのまま残す（名前、有給用西暦、チェックボックス状態）
  displayTimeCards();
}

// "YYYY-MM-DD" 形式の日付から "MM-DD" を返す
